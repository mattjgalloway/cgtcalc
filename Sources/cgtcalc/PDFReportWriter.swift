...
enum PDFReportFormatterError: Error, CustomStringConvertible {
  ...
  var description: String {
    switch self {
    case .failedToCreatePDFData:
      return "Unable to create PDF data."
    }
  }
}
...
struct PDFReportFormatter: ReportFormatter {
  ...
  func render(_ result: CalculationResult) throws -> FormattedReport {
    ...
    var layout = PDFLayout(context: context, pageWidth: self.pageWidth, pageHeight: self.pageHeight, margin: self.margin)
    layout.beginPage()
  ...
  private func drawTaxYearDetails(_ summaries: [TaxYearSummary], layout: inout PDFLayout) {
    ...
    for summary in ordered {
      ...
      layout.drawText("Tax Year \(summary.taxYear.label)", x: self.margin, y: layout.y, font: self.bodyBoldFont, color: self.colorPrimary)
      layout.y += 15
      ...
      for (index, disposal) in summary.disposals.enumerated() {
        ...
        for match in disposal.bedAndBreakfastMatches {
          let label = self.utcCalendar.isDate(match.buyTransaction.date, inSameDayAs: disposal.sellTransaction.date) ? "Same day" : "Bed & breakfast"
  ...
  private func drawTaxReturnInformation(_ summaries: [TaxYearSummary], layout: inout PDFLayout) {
    ...
    for summary in ordered {
      ...
      let specialHeight: CGFloat = specialLine.map { layout.measureWrappedTextHeight($0, width: self.contentWidth - 20, lineHeight: 11) + 8 } ?? 0
      ...
      layout.drawText("Tax Year \(summary.taxYear.label)", x: cardX + 8, y: cardY + 4, font: self.bodyBoldFont, color: self.colorPrimary)
      ...
      for (index, row) in rows.enumerated() {
        ...
        layout.drawRightAlignedText(row.value, maxX: cardX + self.contentWidth - 8, y: rowY + 3, font: self.bodyBoldFont, color: self.colorText)
        rowY += rowHeight
      }
  ...
  func detailedCalculationLine(for disposal: Disposal) -> String {
    ...
    if !disposal.bedAndBreakfastMatches.isEmpty {
      for match in disposal.bedAndBreakfastMatches {
        ...
        let eventAdjustmentPart = match.eventAdjustment != 0 ? " + \(self.decimalString(self.rounded(match.eventAdjustment, scale: 2)))" : ""
  ...
  func taxReturnEntry(for summary: TaxYearSummary, disposalsCount: Int? = nil) -> TaxReturnEntry {
    ...
    {
      let gainsTo = summary.disposals.filter { $0.gain > 0 && $0.sellTransaction.date <= cutoff }.reduce(Decimal(0)) { $0 + $1.gain }
      let gainsAfter = summary.disposals.filter { $0.gain > 0 && $0.sellTransaction.date > cutoff }.reduce(Decimal(0)) { $0 + $1.gain }
  ...
  private func drawMonospaceLines(_ lines: [String], sectionTitle: String, layout: inout PDFLayout) {
    ...
    for line in lines {
      let lineHeightNeeded = layout.measureWrappedTextHeight(line, width: self.contentWidth, lineHeight: lineHeight) + 2
