...
final class CalculationTimelineGroupingTests: XCTestCase {
  func testCanonicalEventOrderPrecedesSourceOrderAndUUID() throws {
    let earlyID = try XCTUnwrap(UUID(uuidString: "00000000-0000-0000-0000-000000000001"))
    let lateID = try XCTUnwrap(UUID(uuidString: "00000000-0000-0000-0000-000000000002"))
    let capitalReturn = AssetEvent(
      id: earlyID,
      sourceOrder: 0,
      date: TestSupport.date("15/06/2020"),
      asset: "TEST",
      kind: .capitalReturn(amount: 100, value: 3))
    let dividend = AssetEvent(
      id: lateID,
      sourceOrder: 1,
      date: TestSupport.date("15/06/2020"),
      asset: "TEST",
      kind: .dividend(amount: 100, value: 5))

    XCTAssertEqual(
      [capitalReturn, dividend].sorted(by: CalculationTimeline.assetEventSortsBefore).map(\.id),
      [lateID, earlyID])
  }

  func testEqualPriorityEventsUseFixedUUIDAsFinalTieBreak() throws {
    ...
    let first = AssetEvent(
      id: firstID,
      date: TestSupport.date("15/06/2020"),
      asset: "TEST",
      kind: .dividend(amount: 50, value: 2))
    let second = AssetEvent(
      id: secondID,
      date: TestSupport.date("15/06/2020"),
      asset: "TEST",
      kind: .dividend(amount: 50, value: 3))

    XCTAssertEqual(
      [second, first].sorted(by: CalculationTimeline.assetEventSortsBefore).map(\.id),
      [firstID, secondID])
  }
  ...
  func testGroupsSameAssetDayAndTypeBySummingAmountAndValue() {
    ...
    let grouped = CalculationTimeline.groupDistributions(events)
  ...
  func testKeepsDifferentAssetsAndDaysSeparate() {
    ...
    XCTAssertEqual(CalculationTimeline.groupDistributions(events).count, 3)
  }
}
