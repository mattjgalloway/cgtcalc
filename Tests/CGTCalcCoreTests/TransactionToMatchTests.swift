...
class TransactionToMatchTests: XCTestCase {
  func testSplitSuccess() throws {
    ...
    let splitAmount = try XCTUnwrap(Decimal(string: "2.123"))
  ...
  func testSplitTwiceSuccess() throws {
    ...
    let split1Amount = try XCTUnwrap(Decimal(string: "10.222"))
    ...
    let split2Amount = try XCTUnwrap(Decimal(string: "5.431"))
  ...
  func testSplitTooMuchFails() throws {
    let transaction = ModelCreation.transaction(.Buy, "15/08/2020", "Foo", "12.345", "1.2345", "12.5")
    let acquisition = TransactionToMatch(transaction: transaction)
    let splitAmount = try XCTUnwrap(Decimal(string: "100"))
  ...
  func testOffsetWorks() throws {
    ...
    try acquisition.addOffset(amount: XCTUnwrap(Decimal(string: "10.0")))
    ...
    try acquisition.addOffset(amount: XCTUnwrap(Decimal(string: "93.982")))
    ...
    try acquisition.subtractOffset(amount: XCTUnwrap(Decimal(string: "10.0")))
    ...
    try acquisition.subtractOffset(amount: XCTUnwrap(Decimal(string: "93.982")))
    XCTAssertEqual(acquisition.price, Decimal(string: "100.0"))
    XCTAssertEqual(acquisition.value, Decimal(string: "1000.0"))
    XCTAssertEqual(acquisition.offset, Decimal.zero)
  }
